...
import { WishElement, ElementProperties, InteractiveElement, PropertyDefinition } from '@/types/templates';
import { Button } from '@/components/ui/Button';
import { ColorPicker } from '@/components/ui/ColorPicker';
import { PremiumPropertyWrapper, PremiumPropertyLabel, PremiumBadge } from '@/components/ui/PremiumPropertyWrapper';
...
export function ElementPropertiesPanel({
  ...
  const [localCustomBackgroundColor, setLocalCustomBackgroundColor] = React.useState(customBackgroundColor);
  const [animationState, setAnimationState] = React.useState({ startAnimation: false, resetAnimation: false });
  ...
  const themes = [
    { value: 'purple', name: 'Purple Dream', gradient: 'from-purple-400 to-pink-400', preview: 'bg-gradient-to-br from-purple-400 to-pink-400' },
    { value: 'ocean', name: 'Ocean Blue', gradient: 'from-blue-400 to-cyan-400', preview: 'bg-gradient-to-br from-blue-400 to-cyan-400' },
    { value: 'sunset', name: 'Sunset', gradient: 'from-orange-400 to-pink-400', preview: 'bg-gradient-to-br from-orange-400 to-pink-400' },
    { value: 'forest', name: 'Forest Green', gradient: 'from-green-400 to-emerald-400', preview: 'bg-gradient-to-br from-green-400 to-emerald-400' },
    { value: 'royal', name: 'Royal Gold', gradient: 'from-yellow-400 to-orange-400', preview: 'bg-gradient-to-br from-yellow-400 to-orange-400' },
    { value: 'midnight', name: 'Midnight', gradient: 'from-indigo-900 to-purple-900', preview: 'bg-gradient-to-br from-indigo-900 to-purple-900' },
    { value: 'cotton-candy', name: 'Cotton Candy', gradient: 'from-pink-300 to-purple-300', preview: 'bg-gradient-to-br from-pink-300 to-purple-300' },
  ...
  const musicLibrary = [
    ...
    { id: 'celebration', name: 'Celebration', url: '/audio/celebration.mp3', duration: '2:45', category: 'celebration' },
    { id: 'peaceful', name: 'Peaceful Melody', url: '/audio/peaceful.mp3', duration: '4:20', category: 'ambient' },
    { id: 'upbeat', name: 'Upbeat Joy', url: '/audio/upbeat.mp3', duration: '2:10', category: 'celebration' }
  ];
  ...
  const transitions = [
    ...
    { value: 'slide-left', name: 'Slide Left', description: 'Slide from right' },
    { value: 'slide-right', name: 'Slide Right', description: 'Slide from left' },
    { value: 'zoom-in', name: 'Zoom In', description: 'Scale up from center' },
    { value: 'zoom-out', name: 'Zoom Out', description: 'Scale down to center' },
        ...
        <div className="p-4 space-y-6">
          ...
          {showCanvasSettings && (
            <div className="border-b pb-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">🎨 Canvas Settings</h3>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {themes.map((themeOption) => (
                    ...
                    >
                      <div className={`w-full h-8 rounded mb-2 ${themeOption.preview}`}></div>
                      <div className="text-xs font-medium text-gray-800">{themeOption.name}</div>
                    </button>
                  ))}
                </div>
          ...
          {element && !showCanvasSettings && (
            <div>
              <h3 className="text-lg font-semibold text-gray-800 mb-4">⚙️ Element Properties</h3>
  ...
  function renderElementProperties() {
    ...
    const elementDefinition = getAllElements().find(el => el.id === element.elementType);
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-2 text-center font-medium">
                  ✨ Start the balloon floating animation or reset to initial state
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handlePropertyChange('balloonSize', parseInt(e.target.value))}
                className="w-full"
              />
              <div className="text-xs text-gray-500 mt-1">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Assign images to individual balloons. Leave empty to show nothing when popped.
                </p>
                
                {/* Individual balloon image inputs */}
                <div className="space-y-2">
                  {Array.from({ length: Math.min(properties.numberOfBalloons || 5, 20) }, (_, index) => (
                    <div key={index} className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        onChange={(e) => handlePropertyChange(`balloonImage${index}`, e.target.value || null)}
        ...
        return (
          <div className="space-y-4">
            ...
            {propertyDefinitions.map((propDef: PropertyDefinition) => {
              const currentValue = properties[propDef.name] ?? propDef.defaultValue;
              ...
              return (
                <div key={propDef.name} className="relative pb-3">
                  <PremiumPropertyWrapper
                    ...
                    upgradeMessage={propDef.upgradeMessage || 'Upgrade to access this feature'}
                    ...
                    <div className="space-y-2">
                      ...
                      {propDef.type === 'text' && (
                        <input
                          type="text"
                          value={currentValue}
                          onChange={(e) => handlePropertyChange(propDef.name, e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                      ...
                      {propDef.type === 'select' && (
                        <select
                          ...
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                        >
                          {propDef.options?.map((option: { value: string; label: string; isPremium?: boolean }) => (
                            <option key={option.value} value={option.value}>
                              {option.label}
                              {option.isPremium && !isUserPremium && ' ✨ (Premium)'}
                            </option>
                          ))}
                        </select>
                      )}
                      ...
                      {propDef.type === 'range' && (
                        <>
                          <input
                            ...
                            step={propDef.step}
                            value={currentValue}
                            onChange={(e) => handlePropertyChange(propDef.name, parseInt(e.target.value))}
                            className="w-full"
                          />
                          <div className="text-xs text-gray-500 mt-1">
                            {currentValue}
                            {propDef.name.includes('Size') && 'px'}
                            {propDef.name.includes('Speed') && ' (Slow: ' + propDef.min + ', Fast: ' + propDef.max + ')'}
                            {propDef.name.includes('Duration') && 'ms'}
                            {propDef.name.includes('Padding') && 'px (None: ' + propDef.min + 'px, Large: ' + propDef.max + 'px)'}
                          </div>
                        </>
                      )}
                      ...
                      {propDef.type === 'checkbox' && (
                        <label className="flex items-center">
                          <input
                            ...
                            onChange={(e) => handlePropertyChange(propDef.name, e.target.checked)}
