...
export function WishCanvas({
  ...
  const [completedSteps, setCompletedSteps] = React.useState<Set<string>>(new Set()); // Track completed elements
    ...
    if (isPreviewMode) {
      ...
      if (stepSequence && stepSequence.length > 0) {
        if (currentStep < stepSequence.length) {
          ...
          return elements.filter(element => currentStepElements?.includes(element.id) || false);
        }
        return [];
      } else {
  ...
  const renderElement = (element: WishElement) => {
    ...
    const baseClasses = `absolute cursor-pointer transition-all duration-200 ${
      isSelected ? 'ring-2 ring-purple-500 ring-offset-2' : 'hover:ring-2 hover:ring-purple-300'
    }`;
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border h-full flex flex-col">
      ...
      {!isPreviewMode && (
        <div className="p-4 border-b flex-shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-800">Wish Canvas</h3>
      ...
      <div className={`relative bg-gradient-to-br from-purple-50 to-pink-50 overflow-hidden ${isPreviewMode ? 'h-full' : 'flex-1'}`}>
        {/* Background */}
        {customBackgroundColor ? (
          <div 
          ...
          <div className={`absolute inset-0 bg-gradient-to-br ${themeGradients[theme as keyof typeof themeGradients] || themeGradients.purple} opacity-60`} />
        )}
        ...
        <div className="relative w-full h-full p-4 sm:p-6 md:p-8 overflow-auto">
          ...
          {isPreviewMode && (
            <div className="absolute top-4 right-4 z-20">
              <div className="bg-white/90 backdrop-blur-sm rounded-full px-3 py-1 text-sm font-medium text-gray-700 shadow-lg">
                Step {currentStep + 1} of {stepSequence && stepSequence.length > 0 
                  ? stepSequence.length 
                  : elements.filter(el => 
                      el.elementType === 'balloons-interactive' || 
                      el.elementType === 'beautiful-text' ||
          ...
          {isPreviewMode && getVisibleElements().length > 0 && (() => {
            // Use custom step sequence if provided, otherwise auto-generate
            const sequenceToUse: string[][] = stepSequence && stepSequence.length > 0
              ? stepSequence
              : elements.filter(el =>
                  el.elementType === 'balloons-interactive' ||
                  el.elementType === 'beautiful-text' ||
          ...
          {elements.length === 0 && (
            <div className="flex items-center justify-center h-full">
              <div className="text-center text-gray-500 px-4">
                ...
                <h3 className="text-lg sm:text-xl font-semibold mb-2">Start Building Your Wish</h3>
                <p className="text-sm sm:text-base">Add elements from the palette to create your special wish</p>
              </div>
            </div>
          )}

          {/* Rendered Elements */}
          <div className={`transition-opacity duration-300 ${isTransitioning ? 'opacity-0' : 'opacity-100'} min-h-full`}>
