...
export function ElementPalette({ 
  ...
  const getFilteredElements = () => {
    if (selectedCategory === 'selected') {
      ...
      return selectedElements.map((selectedElement, index) => ({
        ...
        icon: selectedElement.elementType === 'balloons-interactive' ? '🎈' : '📝',
        category: 'selected',
  ...
  const handleElementClick = (elementId: string) => {
    if (selectedCategory === 'selected') {
      ...
      const originalElement = selectedElements.find(el => 
        el.id === elementId || `${el.elementType}_${selectedElements.indexOf(el)}` === elementId
      );
      if (originalElement) {
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        <div className="grid grid-cols-2 gap-3">
          {filteredElements.map((element) => {
            const isSelected = selectedCategory === 'selected' 
              ...
              >
                ...
                {isSelected && selectedCategory !== 'selected' && (
                  <div className="absolute top-2 right-2 text-blue-500 text-xs">
                    {selectedElements.filter(selected => selected.id === element.id).length}
                  </div>
                )}
              </button>
            );
